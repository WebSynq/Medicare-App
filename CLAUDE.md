...
### Public booking page 500 fix (May 2026)
`staging.ghwcrm.com/book/tim-arnold` was returning 500 (browser console: minified React error #438 "Objects are not valid as a React child") because `app/src/app/book/[slug]/page.tsx` was using React 19's `use(params)` to unwrap `params` as a Promise — but this is Next.js 14.2.35 + React 18.3.1, where `params` on a client component is a plain object, not a Promise. The SSR pass crashed before the backend was ever called. Fix: swap `use(params)` for `useParams<{ slug: string }>()` from `next/navigation` (the pattern used in `clients/[id]/page.tsx` and every other dynamic page) + `enabled: slug.length > 0` guard on the info query + defensive `(info.meeting_types ?? []).filter((m) => typeof m === "string")` on the AgentHeader badge render and the Step 2 meeting-type select. Backend untouched. `npm run typecheck`/`lint`/`build` clean; `/book/[slug]` 6.8 kB / 182 kB First Load JS. Test floor unchanged (527).

### Round-robin booking audit (May 2026)
The backend has a fully built deficit-weighted round-robin distribution engine in `backend/calendars_router.py`: `pick_and_record_round_robin()` (atomic `$inc` + `$set` on `distribution.assignment_counts.{uid}` and `distribution.last_assigned_at.{uid}`), `_select_round_robin_member()` (sort by `(-deficit, last_assigned_at, index)` for ties), `_build_member_view()` (hydrates members + per-weekday availability check). `Calendar.type = "round_robin"` shape carries `member_ids`, `distribution.weights` (1-5), `assignment_counts`, `last_assigned_at`. `POST /api/calendars` creates round-robin calendars; `GET/PATCH /api/calendars/{id}/distribution` exposes the ledger to owners/admins. The public booking page handles round-robin slugs transparently — `_agent_by_slug()` returns a calendar proxy with `_is_calendar_proxy=true` for `/info`/`/token`/`/slots`, then `create_booking` swaps the proxy for the selected agent at POST time. Frontend has `/appointments/round-robin` admin page (lists round-robin calendars, edits weights via `patchDistribution`). **Missing**: a frontend wizard to create round-robin calendars (the admin page only lists existing ones + edits weights; calendars must currently be created via direct `POST /api/calendars`). Public booking page itself needs no round-robin changes — the proxy abstraction means the page already works for both individual and round-robin slugs.
